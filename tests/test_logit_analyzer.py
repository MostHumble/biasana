...
class TestLogitAnalyzer(unittest.TestCase):
    ...
    def test_get_most_biased_templates(self):
        results = self.analyzer.get_most_biased_templates(
            self.test_contexts, self.test_groups, top_n=2
        )
        ...
        first_disparity = abs(
            results[0].normalized_probabilities["man"]
            - results[0].normalized_probabilities["woman"]
        )
        second_disparity = abs(
            results[1].normalized_probabilities["man"]
    ...
    def test_top_n_validation(self):
        results = self.analyzer.get_most_biased_templates(
            self.test_contexts, self.test_groups, top_n=len(self.test_contexts) + 1
        )
